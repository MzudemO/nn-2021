...
class GWRStrategy:
    ...
    def replay_samples(self):
        ...
        r_weights = np.zeros((self.episodic.num_nodes, size, self.episodic.dimension))
        r_labels = np.zeros(
            (self.episodic.num_nodes, len(self.episodic.num_labels), size)
        )
        for i in range(0, self.episodic.num_nodes):
